  ...
  if (n >= (int)sizeof(HEADER) && header->qr && forward)
    {
       ...
       if ((header->rcode == SERVFAIL || header->rcode == REFUSED || header->rcode == NXDOMAIN) && forward->forwardall == 0)
	 ...
	 {
	   if (header->rcode == SERVFAIL || header->rcode == REFUSED || header->rcode == NXDOMAIN)
	     server = NULL;
	   else
	     {
	 ...
	 everything is broken */
	  ...
	  (header->rcode != REFUSED && header->rcode != SERVFAIL && header->rcode != NXDOMAIN))
	{
...
void receive_query(struct listener *listen, struct daemon *daemon, time_t now)
{
  ...
  if (extract_request(header, (size_t)n, daemon->namebuff, &type))
    ...
    if (daemon->options & OPT_REPLY_DST) {
      /* 
