...
http_get(CONN *C, char *host, int port, char *path)
{
  ...
  char portstr[16];
  char fullpath[4096];
  char cookie[MAX_COOKIE_SIZE+10];
  ...
  if(my.proxy.required){
    sprintf(
      fullpath, "%s://%s:%d%s", C->prot == HTTP?"http":"https", host, port, path 
    );
  } 
  else{
  ...
  if((port==80 && C->prot==HTTP) || (port==443 && C->prot==HTTPS)){
    portstr[0] = '\0';  
  } else {
  ...
  get_cookie_header(pthread_self(), host, cookie); 
  if( C->auth.www ){
    rlen = snprintf(
  ...
  rlen = snprintf(
    ...
    fullpath, protocol, host, portstr,
...
http_post(CONN *C, char *host, int port, char *path, char *data, size_t len)
{
  ...
  if( my.proxy.required ){
   sprintf(
      fullpath, 
      "%s://%s:%d%s", 
      C->prot == 0?"http":"https", host, port, path 
    ); 
  }
  else{
    sprintf(fullpath, "%s", path);
  ...
  rlen = snprintf(
    ...
    (C->auth.proxy==TRUE)?authpxy:"",
    (strlen(cookie) > 8)?cookie:"", 
    my.encoding, my.uagent, my.extra, keepalive, (long)len
  ); 
