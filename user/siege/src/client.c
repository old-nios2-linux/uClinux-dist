...
http_request(CONN *C, URL *U, CLIENT *client)
{
  ...
  if(U->calltype == URL_POST){ 
    if((http_post(C, U->hostname, U->port, U->pathname, U->postdata, U->postlen)) < 0){
      C->connection.reuse = 0;
      socket_close(C);
      return FALSE;
    }
  } else { 
    if((http_get(C, U->hostname, U->port, U->pathname)) < 0){
      C->connection.reuse = 0;
      socket_close(C);
      return FALSE;
    }
   ...
   */
/*
  head     =  http_read_headers(C, U->hostname); 
  if(!head){  C->connection.reuse = 0; socket_close(C); return FALSE; } 
*/

  if((head = http_read_headers(C, U->hostname))==NULL){
    C->connection.reuse = 0; 
    socket_close(C); 
    ...
    case 302:
      ...
      if(my.follow && head->redirect[0]){
        ...
        if(protocol_length(head->redirect) == 0){
          ...
          redirect_url = add_redirect(head->redirect, U->port, U->urlid);
        }
