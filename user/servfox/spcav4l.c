...
static void spcaSetTimeInterval(int fd, struct video_param *videoparam, unsigned short dtime);
static void spcaSetQuality(int fd, struct video_param *videoparam, unsigned char qindex);
...
init_v4l (struct vdIn *vd)
{
  ...
  if(debug) printf ("Camera found: %s \n", vd->videocap.name);
  ...
  erreur = GetVideoPict (vd);
   if (ioctl (vd->fd, VIDIOCGCHAN, &vd->videochan) == -1)
    ...
    if (vd->videochan.name){
      ...
      {
       if(debug) printf ("Bridge not found not a spca5xx Webcam Probing the hardware !!\n");
      vd->cameratype = UNOW;
      }
    }
...
{	/* probe palette and set a default one for unknow cams*/
	...
	if(debug) printf("try palette %d depth %d\n",pict.palette,pict.depth);
	if (ioctl (vd->fd, VIDIOCSPICT, &pict) < 0)
    		{
      		if(debug) printf("Couldnt set palette first try %d \n", pal[i]);
      
    		}
	if (ioctl (vd->fd, VIDIOCGPICT, &pict) < 0)
    		{
      		if(debug) printf("Couldnt get palette %d \n", pal[i]);
      
    		}
	if (pict.palette != pal[i]){
		if(debug) printf("Damned second try fail \n");	
		} else {
		availpal = availpal | masq ;
		if(debug) printf("Available  palette %d \n", pal[i]);
		if (defaut){
		 defaut = 0;
		 //vd->formatIn = pal[i];
...
{	/* probe size and set a default one for unknow cams */
	...
	if(debug) printf("probe size in \n");
	while (size[i] > maxw){
	if(debug) printf("skip size %d x %d\n",size[i],size[i+1]);
		i += 2;
		masq = masq << 1;
		if (i > 13) break;
	}
	...
	if (ioctl (vd->fd, VIDIOCSWIN, &win) < 0) {
      		if(debug) printf ("VIDIOCSWIN reject width %d  height %d \n",
	      		win.width, win.height);	      
    	} else {
	vd->sizeothers = vd->sizeothers | masq;
	 if(debug) printf ("Available Resolutions width %d  heigth %d \n",
	 		win.width, win.height);
	if (defaut)
		{
...
static int checkpalette (struct vdIn *vd)
{
	...
	if ((palette = paletteconvert( needpalette)) < 0){
		if(debug) printf("Invalid palette in check palette fatal !! \n");
		return -1;
	}
	  ...
	  if (ioctl (vd->fd, VIDIOCMCAPTURE, &(vd->vmmap)))
	    ...
	    if(debug) printf("palette not find check palette fatal !! \n");
	    return -1;	
	    }
	    /*populate the change */
	vd->formatIn = palette;
...
static int check_palettesize(struct vdIn *vd ){
	...
	if(debug) printf( "size seem unavailable fatal errors !!\n");
		return -1;
	}
	...
	if(needsize){
		...
		if(sizeconvert( &vd->hdrwidth,&vd->hdrheight,needsize) > 0){
			if(debug) printf( "size not set fatal errors !!\n");
		return -1;
		}
		...
		return -1;
		 ...
		 if(debug) printf ("Damned no match found Fatal errors !!\n");
		 return -1;
	 }
return needsize;	
}
...
/* return Stream_id otherwhise -1 */
  ...
  switch (match)
    ...
    default:
      i = UNOW; // -1;
      if(debug) printf ("Unable to find a StreamId !!\n");
      break;
...
SpcaGetBrightness (struct vdIn * vdin)
{
  if (GetVideoPict (vdin) < 0)
    {
      if(debug) printf (" Error getBrightness \n");
      return 0;
    }
...
SpcaSetBrightness (struct vdIn *vdin, __u8 bright)
{
  ...
  if (SetVideoPict (vdin) < 0)
    {
      if(debug) printf (" Error setBrightness \n");
    }
...
SpcaGetContrast (struct vdIn *vdin)
{
  if (GetVideoPict (vdin) < 0)
    {
      if(debug) printf (" Error getContrast \n");
      return 0;
    }
...
SpcaSetContrast (struct vdIn *vdin, __u8 contrast)
{
  ...
  if (SetVideoPict (vdin) < 0)
    {
      if(debug) printf (" Error setContrast \n");
    }
}
__u8
SpcaGetColors (struct vdIn *vdin)
{
  if (GetVideoPict (vdin) < 0)
    {
      if(debug) printf (" Error getColors \n");
      return 0;
    }
...
SpcaSetColors (struct vdIn *vdin, __u8 colors)
{
  ...
  if (SetVideoPict (vdin) < 0)
    {
      if(debug) printf (" Error setColors \n");
    }
}
...
{ 	
	...
	if (SetVideoPict (vdin) < 0)
    		{
      			if(debug) printf (" Error setVideopict \n");
			return 0;
    		}	
	}
	return bright;	
...
qualityUp(struct vdIn *vdin)
{
	...
	int fd = vdin->fd;
	unsigned char qual = videoparam->quality;
	qual += 1;
	spcaSetQuality(fd,videoparam,qual);
}
void
...
timeUp(struct vdIn *vdin)
{
	struct video_param *videoparam= &vdin->videoparam;
	int fd = vdin->fd;
	unsigned short dtime = videoparam->time_interval;
	dtime+=10;
	spcaSetTimeInterval(fd,videoparam,dtime);
}
void
...
static void spcaSetTimeInterval(int fd, struct video_param *videoparam, unsigned short dtime)
{
	...
	videoparam->time_interval = dtime;
	if(ioctl(fd,SPCASVIDIOPARAM, videoparam) == -1){
