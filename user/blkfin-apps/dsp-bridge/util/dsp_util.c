...
{
	printf("corebld - Blackfin Core B loader\nVersion: $Id: corebld.c 8089 2009-05-21 18:17:21Z vapier $\n");
	exit(EXIT_SUCCESS);
}
...
static void show_usage(int exit_status)
{
	printf(
		...
		"\t-s, --start       start Core B\n"
		"\t-r, --reset       reset coreb\n"
		"\t-t, --stop        stop coreb\n"
		"\t-k, --skip-start  skip starting of Core B -- just load\n"
	);
	exit(exit_status);
}
...
int main(int argc, char *argv[])
{
	...
	bool coreb_reset, coreb_stop, coreb_start;
	...
	while ((i=getopt_long(argc, argv, GETOPT_FLAGS, long_opts, NULL)) != -1) {
		...
		case 's': coreb_start = true; break;
		case 'r': coreb_reset = true; break;
		case 't': coreb_stop = true; break;
		case 'k': skip_coreb_start = true; break;
	...
	fd = open_coreb();
	if (ioctl(fd, CMD_COREB_GET_MMREGION_NUM, &region_num)) {
		close(fd);
		fprintf(stderr, "ioctl CMD_COREB_GET_MMREGION_NUM failed\n");
		return EXIT_FAILURE;
	}
	mem_regions = (struct mem_region *)malloc(region_num * sizeof(*mem_regions));
	for (i = 0; i < region_num; i++) {
		mem_regions[i].index = i;
		ioctl(fd, CMD_COREB_GET_MMREGION, &mem_regions[i]);
		//fprintf(stderr, "mem_regions[%d], 0x%08x - 0x%08x\n", i, (uint)mem_regions[i].start, (uint)mem_regions[i].end);
	}

	if (coreb_stop) {
		if (ioctl(fd, CMD_COREB_STOP, NULL)) {
			close(fd);
			fprintf(stderr, "ioctl CMD_COREB_STOP failed\n");
			return EXIT_FAILURE;
		}
	}
	if (coreb_reset) {
		if (ioctl(fd, CMD_COREB_RESET, NULL)) {
			close(fd);
			fprintf(stderr, "ioctl CMD_COREB_RESET failed\n");
			return EXIT_FAILURE;
		}
	}
	if (coreb_start) {
		if (ioctl(fd, CMD_COREB_START, NULL)) {
			close(fd);
			fprintf(stderr, "ioctl CMD_COREB_START failed\n");
			return EXIT_FAILURE;
		}
	}
	close(fd);
