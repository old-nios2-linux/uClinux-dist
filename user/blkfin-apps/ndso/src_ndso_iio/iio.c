...
int iio_read_device_files(char *device_name, unsigned out)
{
	...
	printf
	    ("<TABLE> <TABLE BORDER=\"5\" BORDERCOLOR=\"#000099\" BORDERCOLORLIGHT=\"#66FFFF\">\n");

	while (ent = readdir(dp), ent != NULL) {
		ret = asprintf(&filename, "%s/%s", dev_dir_name, ent->d_name);
		if (ret < 0) {
			ret = -ENOMEM;
			goto error_close_dir;
		}
		...
		if (S_ISREG(entrystat.st_mode)) {
			...
			fscanf(sysfsfp, "%s\n", (char *) &buf);
			printf
			    ("<TR> <TD WIDTH=\"100\"><FONT FACE=\"Arial\" SIZE=\"5\">  %s </FONT></TD> <TD><FONT FACE=\"Arial\" SIZE=\"5\"> %s </FONT></TD> </TR> \n",
			     ent->d_name, buf);
...
{
	int ret, i, toread = 0, buf_len;

	FILE *fp_ev;
	int fp;

	int num_channels;
	char *dev_dir_name, *buf_dir_name, *trig_dir_name;
	...
	buf_len = info->stime_s.samples * 2;
	...
	iio_enable_selected_channels(dev_dir_name, info->channel_en_mask);
	...
	asprintf(&trig_dir_name, "%strigger%d", iio_dir, trig_num);
...
//      printf("%s %s\n", dev_dir_name, trigger_name);
	...
	ret = asprintf(&buffer_access,
		       "/dev/device%d:buffer0:access0", dev_num);
	if (ret < 0) {
		ret = -ENOMEM;
	...
	} while (toread < buf_len / 2);

	write_sysfs_int("frequency", trig_dir_name, 0);
	...
	for (i = 0; i < read_size / scan_size; i++)
		process_scan(info,
			     data + scan_size * i,
			     infoarray, num_channels, scan_size, tstmp);
