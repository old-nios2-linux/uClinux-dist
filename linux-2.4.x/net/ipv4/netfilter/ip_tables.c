...
static inline int
ip_iface_match(const char *dev, const char *matchdev, const unsigned char *mask)
{
	int i, ret;

	/* Look for ifname matches; this should unroll nicely. */
	for (i = 0, ret = 0; i < IFNAMSIZ/sizeof(unsigned long); i++) {
		ret |= (((const unsigned long *)dev)[i]
			^ ((const unsigned long *)matchdev)[i])
...
#define FWINV(bool,invflg) ((bool) ^ !!(ipinfo->invflags & invflg))
	...
	if (ipinfo->flags & IPT_F_VIA_IN) {
		ret = ip_iface_match(indev, ipinfo->iniface,
				ipinfo->iniface_mask);
#if defined(CONFIG_BRIDGE) || defined(CONFIG_BRIDGE_MODULE)
		if (ret != 0)
			ret = ip_iface_match(physindev, ipinfo->iniface,
					ipinfo->iniface_mask);
#endif
	...
	if (ipinfo->flags & IPT_F_VIA_OUT) {
		ret = ip_iface_match(outdev, ipinfo->outiface,
				ipinfo->outiface_mask);
#if defined(CONFIG_BRIDGE) || defined(CONFIG_BRIDGE_MODULE)
		if (ret != 0)
			ret = ip_iface_match(physoutdev, ipinfo->outiface,
					ipinfo->outiface_mask);
#endif
