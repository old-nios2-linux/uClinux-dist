...
hash_conntrack(const struct ip_conntrack_tuple *tuple)
{
	...
	return (jhash_3words(tuple->src.ip,
	                     ...
	                     (tuple->src.u.first_16 | (tuple->dst.u.first_16<<16)),
		   ...
		   int hooknum)
{
	...
	if (!protocol->new(conntrack, skb, hooknum)) {
...
#endif
#if CONFIG_IP_NF_TARGET_LED
		conntrack->led = expected->expectant->led;
#endif
		  ...
		  enum ip_conntrack_info *ctinfo)
{
	...
	if (!h) {
		h = init_conntrack(&tuple, proto, skb, hooknum);
		if (!h)
			return NULL;
		if (IS_ERR(h))
			return (void *)h;
...
/* Doesn't cover locally-generated broadcast, so not worth it. */
	...
	ret = proto->packet(ct, ctinfo, (*pskb), hooknum);
	if (ret < 0 ) {
