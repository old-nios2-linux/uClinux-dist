 ...
 *
 * Support for FEC controller of ColdFire 5235/5270/5271/5272/5280/5282.
 ...
 *   2004/10/01  - Paul Mecklenburg (paulm@vxitech.com)
 *      - Added support for Broadcom 5221 PHY
 *   2005/08/01  - Mike Lavender (mike@steroidmicros.com)
 *      - Added support for M5208EVB
 */
...
#define CONFIG_FEC_DP83848
#define CONFIG_FEC_BCM5221
#define CONFIG_FEC_KS8721BL
...
/*
 *	Set maximum numbers of FEC devices.
 */
 ...
 */
#if defined(CONFIG_M5272)
static unsigned int fec_hw[] = { (MCF_MBAR+0x840) };
#elif defined(CONFIG_M5235) || defined(CONFIG_M5282) || defined(CONFIG_M5280)
static unsigned int fec_hw[] = { (MCF_MBAR+0x1000) };
#elif defined(CONFIG_M5208)
static unsigned int fec_hw[] = { (MCF_MBAR+0x30000) };
#elif defined(CONFIG_M527x)
static unsigned int fec_hw[] = { (MCF_MBAR+0x1000), (MCF_MBAR+0x1800) };
#else
static unsigned int fec_hw[] = { &(((immap_t *)IMAP_ADDR)->im_cpm.cp_fec) };
#endif


	

static unsigned char	fec_mac_default[] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
...
/*
 * The 527x/528x RX control register also contains maximum frame size bits.
 * Other FEC hardware does not, so we need to take that into account
 * when setting it.
 */
#if defined(CONFIG_M5235) || defined(CONFIG_M527x) || \
    defined(CONFIG_M5280) || defined(CONFIG_M5282) || \
...
fec_timeout(struct net_device *dev)
{
	...
	//fec_restart(dev, 0);  
	// This fec_restart call is an error, as it erroneously 
	// changes duplex while the connection is active
...
/* The National DP83848 is used on the M5208EVB Board                        */

#if defined(CONFIG_FEC_DP83847) || defined(CONFIG_FEC_DP83848)

/* register definitions for the DP8384x */
...
#endif /* CONFIG_FEC_DP83848 */

#endif /* CONFIG_FEC_DP83847 || CONFIG_FEC_DP83848 */
...
#if defined(CONFIG_FEC_BCM5221)
#define MII_BCM5221_IR 0x1A
#define MII_BCM5221_AUX_CSR 0x18

static void mii_parse_bcm5221_sr(uint mii_reg, struct net_device *dev)
{
	struct fec_enet_private *fep = dev->priv;
	volatile uint *s = &(fep->phy_status);
...
static void mii_parse_bcm5221_sr2(uint mii_reg, struct net_device *dev)
{
	struct fec_enet_private *fep = dev->priv;
	volatile uint *s = &(fep->phy_status); 
...
static phy_info_t phy_info_bcm5221 = {
	...
	(const phy_cmd_t []) { /* startup - enable interrupts */
		{ mk_mii_write(MII_BCM5221_IR, 0x4000), NULL },
		{ mk_mii_write(MII_REG_CR, 0x1200), NULL }, /* autonegotiate */
		{ mk_mii_read(MII_REG_SR), mii_parse_bcm5221_sr },
		{ mk_mii_read(MII_REG_SR), mii_parse_bcm5221_sr },
		{ mk_mii_read(MII_BCM5221_AUX_CSR), mii_parse_bcm5221_sr2 },
		{ mk_mii_end, }
	},
	(const phy_cmd_t []) { /* ack_int */
		/* find out the current status */
		{ mk_mii_read(MII_REG_SR), mii_parse_bcm5221_sr }, /* clear etched bit */
		{ mk_mii_read(MII_REG_SR), mii_parse_bcm5221_sr }, /* read status */
		{ mk_mii_read(MII_BCM5221_AUX_CSR), mii_parse_bcm5221_sr2 },
		{ mk_mii_read(MII_BCM5221_IR), NULL /*mii_parse_bcm5221_ir*/ }, /* ack */
		{ mk_mii_end, }
	},
	(const phy_cmd_t []) { /* shutdown - disable interrupts */
		{ mk_mii_write(MII_BCM5221_IR, 0x0000), NULL },
		{ mk_mii_end, }
	},
};
...
#if defined(CONFIG_FEC_KS8721BL)
	&phy_info_ks8721bl,
#endif /* CONFIG_FEC_KS8721BL */
...
fec_set_mac_address(struct net_device *dev, void *p)
{
	...
	*/
	memcpy(fep->mac_addr, addr->sa_data,dev->addr_len);
...
int __init fec_enet_init(struct net_device *dev)
{
	...
	*/
	 ...
	 */
	eap = (unsigned char *)fep->mac_addr;
	for (i=0; (i<6); i++)
		eap[i] = fec_mac_default[i];
	eap[5] = index;

#if defined(CONFIG_COLDFIRE)
#if defined(CONFIG_NETtel) || defined(CONFIG_GILBARCONAP) || \
    defined(CONFIG_SE1100) || defined(CONFIG_HW_FEITH)
	/*
	 * If the MAC is all 1's or 0's, used the default coded one.
	 */
#if defined(CONFIG_GILBARCONAP) || defined(CONFIG_SE1100) || \
...
#else /* not defined(CONFIG_COLDFIRE) */
	iap = bd->bi_enetaddr;
	...
	*/
	...
	j = (index) ? 128 : 64;
        ...
        j = 64 + 13;   /* 5208 module is the same just vectors are offset */
#endif

	/* Setup interrupt handlers. */
	if (request_irq(j+23, fec_enet_interrupt, 0, "fec(TXF)", dev) != 0)
		printk("FEC: Could not allocate FEC(TXF) IRQ(%d+23)!\n", j);
	if (request_irq(j+24, fec_enet_interrupt, 0, "fec(TXB)", dev) != 0)
		printk("FEC: Could not allocate FEC(TXB) IRQ(%d+24)!\n", j);
	if (request_irq(j+25, fec_enet_interrupt, 0, "fec(TXFIFO)", dev) != 0)
		printk("FEC: Could not allocate FEC(TXFIFO) IRQ(%d+25)!\n", j);
	if (request_irq(j+26, fec_enet_interrupt, 0, "fec(TXCR)", dev) != 0)
		printk("FEC: Could not allocate FEC(TXCR) IRQ(%d+26)!\n", j);

	if (request_irq(j+27, fec_enet_interrupt, 0, "fec(RXF)", dev) != 0)
		printk("FEC: Could not allocate FEC(RXF) IRQ(%d+27)!\n", j);
	if (request_irq(j+28, fec_enet_interrupt, 0, "fec(RXB)", dev) != 0)
		printk("FEC: Could not allocate FEC(RXB) IRQ(%d+28)!\n", j);

	if (request_irq(j+29, fec_enet_interrupt, 0, "fec(MII)", dev) != 0)
		printk("FEC: Could not allocate FEC(MII) IRQ(%d+29)!\n", j);
	if (request_irq(j+30, fec_enet_interrupt, 0, "fec(LC)", dev) != 0)
		printk("FEC: Could not allocate FEC(LC) IRQ(%d+30)!\n", j);
	if (request_irq(j+31, fec_enet_interrupt, 0, "fec(HBERR)", dev) != 0)
		printk("FEC: Could not allocate FEC(HBERR) IRQ(%d+31)!\n", j);
	if (request_irq(j+32, fec_enet_interrupt, 0, "fec(GRA)", dev) != 0)
		printk("FEC: Could not allocate FEC(GRA) IRQ(%d+32)!\n", j);
	if (request_irq(j+33, fec_enet_interrupt, 0, "fec(EBERR)", dev) != 0)
		printk("FEC: Could not allocate FEC(EBERR) IRQ(%d+33)!\n", j);
	if (request_irq(j+34, fec_enet_interrupt, 0, "fec(BABT)", dev) != 0)
		printk("FEC: Could not allocate FEC(BABT) IRQ(%d+34)!\n", j);
	if (request_irq(j+35, fec_enet_interrupt, 0, "fec(BABR)", dev) != 0)
...
#else
	{
		...
		j = (index) ? MCFICM_INTC1 : MCFICM_INTC0;

		icrp = (volatile unsigned char *) (MCF_IPSBAR + j +
			MCFINTC_ICR0);
		for (i = 23; (i < 36); i++)
			icrp[i] = 0x23;

		imrp = (volatile unsigned long *) (MCF_IPSBAR + j +
			MCFINTC_IMRH);
		*imrp &= ~0x0000000f;
		imrp = (volatile unsigned long *) (MCF_IPSBAR + j +
			MCFINTC_IMRL);
...
#elif defined(CONFIG_M5280) || defined(CONFIG_M5282)
	...
	*(volatile unsigned char*)(MCF_IPSBAR + MCF5235_GPIO_PFECI2CPAR) |= 0xf0;
#elif defined(CONFIG_M527x)
	*(volatile unsigned char *)(MCF_IPSBAR + 0x100078) |= 0xc0;
	*(volatile unsigned short *)(MCF_IPSBAR + 0x100082) |= 0x0f00;
#if defined(CONFIG_FEC2)
        *(volatile unsigned char *)(MCF_IPSBAR + 0x100079) |= 0xc0;
        *(volatile unsigned short *)(MCF_IPSBAR + 0x100082) |= 0x00a0;
#endif
#endif
...
static void fec_enet_setup_hw_p1( struct net_device *dev )
{
	...
	*/
	...
	/* Set station address.
	*/
	eap = (unsigned char *)&fep->mac_addr[0];
	fecp->fec_addr_low = eap[3] | (eap[2] << 8) | (eap[1] << 16) |
		(eap[0] << 24);
	fecp->fec_addr_high = (eap[5] << 16) | (eap[4] << 24);

	for (i=0; i<6; i++)
		dev->dev_addr[i] = eap[i];
