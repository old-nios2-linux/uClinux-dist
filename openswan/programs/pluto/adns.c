...
master(void)
{
	...
	do {
	    ndes = os_select(maxfd + 1, &readfds, NULL, NULL, NULL);
	...
	{
	    if (OS_FD_ISSET(PLUTO_QFD, &readfds))
	    {
		query();
		ndes--;
	    }
	    ...
	    {
		if (w->busy && OS_FD_ISSET(w->afd, &readfds))
		{
		    answer(w);
		    ndes--;
		}
